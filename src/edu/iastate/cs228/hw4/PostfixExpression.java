...
public class PostfixExpression extends Expression 
{
	...
	{
		String s = Expression.removeExtraSpaces(postfixExpression); 
		String out = "";
	...
	public int evaluate() throws ExpressionFormatException 
    {
    	...
    	char[] ary = super.postfixExpression.toCharArray();
    	
    	for (int j = 0; j < ary.length; j++)
    	{
    		if (Expression.isInt(String.valueOf(ary[j])))
    		{
    			String s = String.valueOf(ary[j]);
    			int c = j;
    			
    			while (Expression.isInt(String.valueOf(ary[c + 1])))
    			{
    				s += String.valueOf(ary[c]);
    				c++;
    			}
    		...
    		else if (Expression.isVariable(ary[j]))
    		{
    			...
    			{
    				int x = (int) super.varTable.get(c);
    				
    				operandStack.push(x);
    			}
    			else
    			{
    				throw new ExpressionFormatException("Variable " + String.valueOf(c) + " was not assigned a value");
    			}
    		}
    		else if (Expression.isOperator(ary[j]))
    		{
    			...
    			if (ary[j] == '/' && rightOperand == 0)
    				throw new ExpressionFormatException("Divide by Zero");
    			
    			if (ary[j] == '%' && rightOperand == 0)
    				throw new ExpressionFormatException("Divide by Zero");
    			
    			if (ary[j] == '^' && rightOperand == 0 && leftOperand == 0)
    				throw new ExpressionFormatException("0^0");
    			
    			operandStack.push(compute(ary[j]));
    		}
    		else if (ary[j] == ' ')
    		{
